...
class SquashedGaussianMLPActor(nn.Module):
    ...
    def forward(self, obs, deterministic=False, with_logprob=True):
        ...
        if deterministic:
            # Used for evaluating policy at test time.
            pi_action = mu
        else:
            pi_action = pi_distribution.rsample()

        if with_logprob:
            # Compute log prob from Gaussian, then apply correction for
            # Tanh squashing (see SAC paper Appendix C).
            logp_pi = pi_distribution.log_prob(pi_action).sum(axis=-1)
            logp_pi -= (
                2
                * (np.log(2) - pi_action - F.softplus(-2 * pi_action))
            ).sum(axis=1)
        else:
            logp_pi = None
